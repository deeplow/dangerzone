...
mypy-host:
	poetry run mypy $(MYPY_ARGS) dangerzone

mypy-container:
	poetry run mypy $(MYPY_ARGS) container

mypy-tests:
	poetry run mypy $(MYPY_ARGS) tests
...
.PHONY: test
test:
	poetry run ./dev_scripts/pytest-wrapper.py -v --cov --ignore dev_scripts
